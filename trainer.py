...
class Trainer:
  def __init__(self, class_, party_data):
    ...
    self.__name = trainer_id_to_name[self.__class_]
    self.__party = []
  ...
  def send_out_mon(self, which_mon = None):
    ...
    print(self.__name, "sent out", which_mon.get_name() + '!')
  ...
  def move_choice(self):
    ...
    if self.__class_ == PLAYER:
      return self.get_mon_in_battle().get_moveset()[int(input())]
    else:
      return self.mon_in_battle.get_usable_moves()[random.randint(1,len(self.mon_in_battle.get_usable_moves()))]
